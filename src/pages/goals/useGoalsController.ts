...
import { setActiveCycle, setActivePerson } from '@/lib/goals/store'
import type {
  DemoPerson,
  Goal,
  GoalsSnapshot,
  ...
  const actions = useMemo<GoalsControllerActions>(() => {
    return {
      ...
      async duplicateGoal(targetSubjectId, goalId) {
        ...
        const sourceTitle =
          source.description.trim() || `Untitled goal`
